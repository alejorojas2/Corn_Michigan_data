...
packages <- c("ggplot2","RColorBrewer","grid","gridExtra","plyr","lme4","lsmeans","knitr","tidyr","dplyr", "MASS", "magrittr", "reshape2", "FactoMineR", "cowplot","stringr")
ipak(packages)

#require(devtools)
#install_github('vqv/ggbiplot')

setwd("~/Google Drive File Stream/My Drive/MSU/Chilvers Lab/01_Projects/11_Corn_michigan/Pathogenicity/")
...
#Reading the file
corn_data <- read.csv(file = "Corn_clean_data.txt", sep = "\t")
...
(plot_wp1 <- ggplot(corn_final, 
  ...
  geom_errorbar(wp_limits, width=0.2) + guides(alpha=FALSE) +
  scale_color_manual(values = c("#1f78b4", "#33a02c")) + 
...
(plot_ph1 <- ggplot(corn_final, 
  ...
  geom_errorbar(ph_limits, width=0.2) + guides(alpha=FALSE) +
  scale_color_manual(values = c("#1f78b4", "#33a02c")) + 
  scale_alpha_discrete(range = c(0.5, 0.9)) + theme_gray(base_size = 16) +
  theme(axis.text.x=element_text(angle=60, hjust = 1, vjust = 1, face="italic"), 
        axis.text.y=element_text(angle=90, hjust = 0.5), 
